...
class Simpanan extends OperatorController {
	...
	public function listp($id = null) {
		...
		$this->data['pokok'] = $this->db->select("SUM(jumlah) AS simpanan_pokok")
			->where('d.anggota_id', ''.$id.'')
			->where('d.jenis_id', '40')
			->where('d.dk', 'D')
			->get("tbl_trans_sp d")	
			->row();
		
		$this->data['wajib']= $this->db->select("SUM(jumlah) AS simpanan_wajib")
			->where('d.anggota_id', ''.$id.'')
			->where('d.jenis_id', '41')
			->where('d.dk', 'D')
			->get("tbl_trans_sp d")	
		...
		$this->data['sukarela']= $this->db->select("SUM(jumlah) AS simpanan_sukarela")
			->where('d.anggota_id', ''.$id.'')
			->where('d.jenis_id', '32')
			->where('d.dk', 'D')
			->get("tbl_trans_sp d")	
		...
		$this->data['penarikan_pokok'] = $this->db->select("SUM(jumlah) AS penarikan_pokok")
		...
		$this->data['penarikan_wajib']= $this->db->select("SUM(jumlah) AS penarikan_wajib")
			->where('k.anggota_id', ''.$id.'')
			->where('k.jenis_id', '41')
			->where('k.dk', 'K')
			->get("tbl_trans_sp k")	
		...
		$this->data['penarikan_sukarela']= $this->db->select("SUM(jumlah) AS penarikan_sukarela")
			->where('k.anggota_id', ''.$id.'')
			->where('k.jenis_id', '32')
			->where('k.dk', 'K')
			->get("tbl_trans_sp k")	
		...
		$this->data['total']= $this->db->select("SUM(jumlah) AS simpanan_total")
			->where('d.anggota_id', ''.$id.'')
			->where('d.dk', 'D')
			->get("tbl_trans_sp d")	
			->row();
			
		$this->data['penarikan_total']= $this->db->select("SUM(jumlah) AS penarikan_total")
			->where('k.anggota_id', ''.$id.'')
			->where('k.dk', 'K')
			->get("tbl_trans_sp k")	
			->row();
	...
	function ajax_list($id = null) {
		...
		$data   = $this->simpanan_m->get_data_transaksi_ajax($offset,$limit,$search,$sort,$order, $id);
	...
	function cetak_laporan() {
		...
		<tr class="header_kolom">
			...
			<th class="h_tengah" style="width:13%;"> Area </th>
		...
		$no =1;
		$jml_simpanan = 0;
		$jml_simpanan_sukarela = 0;
		$jml_simpanan_pokok = 0;
		$jml_simpanan_wajib = 0;
		...
		foreach ($simpanan as $row) {
			...
			if($row->jenis_id == 32 ){
				$jml_simpanan_sukarela += $row->jumlah;
			}
			
			if($row->jenis_id == 40){
				$jml_simpanan_pokok += $row->jumlah;
			}
			
			if($row->jenis_id == 41){
				$jml_simpanan_wajib += $row->jumlah;
			}
		...
		<tr>
			<td colspan="6" class="h_tengah"><strong> Jumlah Pokok </strong></td>
			<td class="h_kanan"> <strong>'.number_format($jml_simpanan_pokok).'</strong></td>
		</tr>
		<tr>
			<td colspan="6" class="h_tengah"><strong> Jumlah Wajib </strong></td>
			<td class="h_kanan"> <strong>'.number_format($jml_simpanan_wajib).'</strong></td>
		</tr>
		<tr>
			<td colspan="6" class="h_tengah"><strong> Jumlah Sukarela </strong></td>
			<td class="h_kanan"> <strong>'.number_format($jml_simpanan_sukarela).'</strong></td>
		</tr>
		<tr>
