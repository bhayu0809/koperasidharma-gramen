...
class Angsuran extends OperatorController {
	...
	function ajax_list($id = NULL) {
		...
		foreach ($data['data'] as $r) {
			...
			$rows[$i]['jml_pokok_angsuran'] = number_format($r->jml_pokok_angsuran);
			$rows[$i]['jml_jasa_angsuran'] = number_format($r->jml_jasa_angsuran);
	...
	function get_ags_ke($master_id) {
		...
		if($total_tagihan <= 0){
			$status_lunas = "Lunas";
			$this->angsuran_lunas_m->auto_status_lunas($master_id);
		}
		// DENDA
		$denda = 0;
		$denda_semua = 0;
