...
class Anggota extends OperatorController {
	...
	public function index() {
		...
		$crud->columns('id_anggota','identitas','nama','jk','alamat','kota','jabatan_id','departement','tgl_daftar','aktif');
		$crud->fields('nik','nama','identitas','jk', 'tmp_lahir','tgl_lahir','status','departement','pendidikan','pekerjaan','agama','alamat','alamat_kantor_anggota','kota','notelp','tgl_daftar', 'jabatan_id','pass_word','aktif','file_pic','data_penjamin','nama_penjamin','nik_penjamin','no_telpon_penjamin','alamat_penjamin','alamat_kantor_penjamin','pekerjaan_penjamin','pendidikan_penjamin','agama_penjamin');
		//$crud->unset_delete();
		$crud->display_as('id_anggota','ID Anggota');
		$crud->display_as('identitas','Kode Anggota');
		...
		$crud->display_as('departement','Area');
		...
		$check_url_edit = strpos(current_url(),"edit");
		if($check_url_edit){
			$crud->required_fields('nama','identitas','tmp_lahir','tgl_lahir','jk','alamat','kota','jabatan_id','tgl_daftar','aktif');
		}else{
			$crud->required_fields('nama','identitas','tmp_lahir','tgl_lahir','jk','alamat','kota','jabatan_id','tgl_daftar','aktif');		
		}
		$crud->unset_texteditor('alamat','alamat_kantor_anggota','alamat_penjamin','alamat_kantor_penjamin');
		$crud->field_type('no_rek','invisible'); 
		$crud->add_action('Pinjaman', '', 'pinjaman/listp', '');
		$crud->add_action('Simpanan', '', 'simpanan/listp', '');
		$crud->add_action('Bayar Angsuran', '', 'bayar/listp', '');
		$crud->add_action('Pelunasan', '', 'pelunasan/listp', '');
		$crud->add_action('Penarikan', '', 'penarikan/listp', '');
		...
		$this->db->from('mst_hubungan');
		$query = $this->db->get();
		if($query->num_rows()>0){
			$result = $query->result();
			foreach ($result as $val) {
				$data_penjamin[$val->hubungan] = $val->hubungan;
			}
		} else {
			$data_penjamin = array('' => '-');
		}
		$crud->field_type('data_penjamin','dropdown',$data_penjamin);
		
		$crud->display_as('data_penjamin','Penjamin');
		...
		$crud->field_type('agama_penjamin','dropdown',
			array('Islam' => 'Islam',
				'Katolik' => 'Katolik',
				'Protestan' => 'Protestan',
				'Hindu' => 'Hindu',
		...
		// DEPARTEMENT
		$this->db->select('no_ordered,nama_area');
		$this->db->from('mst_area')->order_by('no_ordered');
		$query = $this->db->get();
		
		if($query->num_rows()>0){
			$result = $query->result();
			foreach ($result as $val) {
				$area[$val->nama_area] = $val->nama_area;
			}
		} else {
			$area = array('' => '-');
		}
		$crud->field_type('departement','dropdown',$area);
		...
		if($query->num_rows()>0){
			$result = $query->result();
			foreach ($result as $val) {
				$pendidikan[$val->nama_pendidikan] = $val->nama_pendidikan;
			}
		} else {
			$pendidikan = array('' => '-');
		}
		$crud->field_type('pendidikan','dropdown',$pendidikan);
		$crud->field_type('pendidikan_penjamin','dropdown',$pendidikan);
		...
		$crud->field_type('pekerjaan_penjamin','dropdown',$kerja);
		...
		$crud2->set_table('tbl_anggota');
		$crud2->set_subject('Data Anggota');
