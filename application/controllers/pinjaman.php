...
class Pinjaman extends OperatorController {
	...
	public function listp($id = null) {
		$this->data['anggota'] = '';
		if(!empty($id)){
			$this->data['anggota'] = $this->general_m->get_data_anggota($id);   	
		}
		
		$this->data['judul_browser'] = 'Pinjaman '. $this->data['anggota']->nama;
		$this->data['judul_utama'] = 'Pinjaman '. $this->data['anggota']->nama;
		$this->data['judul_sub'] = 'Pinjaman '. $this->data['anggota']->nama;
	...
	function ajax_list($id = null) {
		...
		$data   = $this->pinjaman_m->get_data_transaksi_ajax($offset,$limit,$search,$sort,$order,$id);
		$i	= 0;
		$rows   = array(); 
		foreach ($data['data'] as $r) {
			...
			$sisa_jasa = $r->total_bunga_pinjaman - $jml_bayar->jasa_dibayar;
			$sisa_pokok = $r->jumlah - $jml_bayar->pokok_dibayar;
			...
			$rows[$i]['anggota_nama'] = ($jml_bayar->angsuran_ke > $r->lama_angsuran ? "<span style='background:red'>".$anggota->nama."</span>" : $anggota->nama);
			...
			$rows[$i]['color_late'] = "style='background:red'";
			$rows[$i]['jumlah'] = number_format(nsi_round($r->jumlah));
			
			if(in_array($r->lama_angsuran,array(5,10,15))){
				$satuan = "Bulan";
				$dateadd = "month";
			}else{
				$satuan =  "Minggu";
			...
			date_add($date,date_interval_create_from_date_string($r->lama_angsuran." ".$dateadd));
			...
			$rows[$i]['hitungan'] = '<table>
						...
						<tr>
							...
							<td width="10px" align="center"> : </td>
							<td width="75px" align="right">'.$r->lama_angsuran.' '.$satuan.'</td>
						</tr>
						<tr>
						...
						<tr>
							<td width="100px" align="left">Sisa Pokok</td> 
							<td width="10px" align="center"> : </td>
							<td width="75px" align="right">'.number_format(nsi_round($sisa_pokok)).'</td>
						</tr>
						<tr>
							<td width="100px" align="left">Tanggal Tempo</td> 
							<td width="10px" align="center"> : </td>
							<td width="75px" align="right">'.$txt_tanggal_t.'</td>
						</tr>
						</table>';
			$rows[$i]['tagihan'] = '<table>
						...
						<tr>
							<td width="100px" align="left">Bunga</td> 
							<td width="10px" align="center"> : </td>
							<td width="75px" align="right">'.$r->bunga.
							'</td>
						</tr>						
						<tr>
						...
						<tr>
							<td width="100px" align="left">Sisa Jasa</td> 
							<td width="10px" align="center"> : </td>
							<td width="75px" align="right">'.number_format(nsi_round($sisa_jasa)).'</td>
						</tr>
						<tr '.($jml_bayar->angsuran_ke > $r->lama_angsuran ? "style='color:red;font-weight:bold'" : "").'>
							<td width="100px" align="left">Kelancaran Bayar</td> 
							<td width="10px" align="center"> : </td>
							<td width="75px" align="right">'.($jml_bayar->angsuran_ke > $r->lama_angsuran ? "<span style='color:red'>Telat</span>" : "Lancar").'</td>
						</tr>
						</table>';
