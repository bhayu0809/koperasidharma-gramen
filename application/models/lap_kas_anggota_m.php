...
//ambil data pinjaman header berdasarkan ID peminjam
	...
	public function transaksi_sp($anggota_id = null){
		$simpanan_pokok = $this->db->select("SUM(jumlah) AS jumlah")
			->where('h.jenis_id', '40')
			->where('h.anggota_id', ''.$anggota_id.'')
			->get("tbl_trans_sp h")	
			->row();
		...
		$pokok = (isset($simpanan_pokok->jumlah) ? $simpanan_pokok->jumlah : 0 );
		$wajib = (isset($simpanan_wajib->jumlah) ? $simpanan_wajib->jumlah : 0 );
		$sukarela = (isset($simpanan_sukarela->jumlah) ? $simpanan_sukarela->jumlah : 0 );
