...
class Simpanan_m extends CI_Model {
	...
	function get_data_transaksi_ajax($offset, $limit, $q='', $sort, $order, $id = null) {
		$sql = "SELECT tbl_trans_sp.*,tbl_anggota.id AS anggota_id  FROM tbl_trans_sp INNER JOIN tbl_anggota ON (tbl_trans_sp.anggota_id = tbl_anggota.id)";
		$where = "WHERE dk='D'";
		if(!empty($id)){
		...
		if(is_array($q)) {
			if($q['kode_transaksi'] != '') {
				...
				if($q['cari_simpanan'] != '') {
					$where .=" AND jenis_id = '".$q['cari_simpanan']."%' ";
				}

				if($q['tgl_dari'] != '' && $q['tgl_sampai'] != '') {
					$where .=" AND DATE(tgl_transaksi) >= '".$q['tgl_dari']."' ";
					$where .=" AND DATE(tgl_transaksi) <= '".$q['tgl_sampai']."' ";
				}
			}
		}
