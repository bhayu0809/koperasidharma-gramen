...
class Pinjaman_m extends CI_Model {
		...
		if($row) {
			...
			for ($i=1; $i <= $row->lama_angsuran; $i++) { 
				...
				*/
				...
				$odat['bunga_pinjaman'] = $row->bunga_pinjaman;
				$odat['jumlah_ags'] =  ($row->bunga_pinjaman + $row->pokok_angsuran);
	...
	function get_data_transaksi_ajax($offset, $limit, $q='', $sort, $order, $id = null) {
		$sql = "SELECT v_hitung_pinjaman.* FROM v_hitung_pinjaman INNER JOIN tbl_anggota ON (v_hitung_pinjaman.anggota_id = tbl_anggota.id) ";
		$where = " WHERE dk = 'K'";
		if(!empty($id)){
	...
	function lap_data_pinjaman() {
		...
		if(is_array($q)) {
			if($q['kode_transaksi'] != '') {
				...
				$q['kode_transaksi'] = str_replace('AG', '', $q['kode_transaksi']);
				$q['kode_transaksi'] = $q['kode_transaksi'] * 1;
				$sql .=" AND (anggota_id LIKE '".$q['kode_transaksi']."') ";
			} 
			if($q['cari_status'] != '') {
