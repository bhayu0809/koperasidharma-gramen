...
$tagihan = ($row_pinjam->pokok_angsuran +  $row_pinjam->bunga_pinjaman + $row_pinjam->biaya_adm) * $row_pinjam->lama_angsuran;
$dibayar = $hitung_dibayar->total;
$angsuran_ke = $hitung_dibayar->angsuran_ke;
$jml_denda=$hitung_denda->total_denda;
$sisa_bayar = $tagihan - $dibayar;
$total_bayar = $sisa_bayar + $jml_denda;
$sisa_jasa = $row_pinjam->total_bunga_pinjaman - $hitung_dibayar->jasa_dibayar;
$sisa_pokok = $row_pinjam->jumlah - $hitung_dibayar->pokok_dibayar;

?>

<!-- menu atas -->
<div class="callout callout-danger">
<!--<code>Klik <strong>Validasi Lunas</strong> untuk melakukan Pelunasan dan Pembayaran Tagihan</code>-->
</div>
...
<div class="box box-solid box-primary">
			...
			<tr>
				...
				<td>
					...
					<?php
							...
							if(in_array($row_pinjam->lama_angsuran,array(5,10,15))){
								$satuan = "Bulan";
								$dateadd = "month";
							}else{
								$satuan =  "Minggu";
							...
							$date=date_create($row_pinjam->tgl_pinjam);
							date_add($date,date_interval_create_from_date_string($row_pinjam->lama_angsuran." ".$dateadd));
							$tanggal_tempo = date_format($date,"Y-m-d");
							$tanggal_arr = explode(' ', $tanggal_tempo);
							$txt_tanggal_t = jin_date_ina($tanggal_arr[0], 'full');
							//$tanggal_tempo = $row_pinjam->tgl_pinjam;
						?>	
					<tr>
						<td> Tanggal Tempoe</td>
						<td> : </td>
						<td> <?php 
								echo $txt_tanggal_t;
							  ?>
					...
					<tr>
						<td> Lama Pinjaman</td> 
						<td> : </td>
						<td> <?php echo $row_pinjam->lama_angsuran." ".$satuan; ?> </span></td>
						
					</tr>
					<tr>
						<td> Kelancaran Bayar</td> 
						<td> : </td>
						<?php
							$tanggal_bayar = date_create($hitung_dibayar->lama_angsuran);
							$tanggal_tempo = date_create($tanggal_tempo);
							
							
						?>
						<td> <?php echo $hitung_dibayar->angsuran_ke.($tanggal_bayar > $tanggal_tempo ? " <span style='color:red'>Telat</span>" : " Lancar" ); ?> </span></td>
					</tr>
					</table>
				</td>
				<td>
					...
					<tr>
						...
						<td> : </td>
						<td class="h_kanan"><?php echo number_format(nsi_round(($row_pinjam->pokok_angsuran +  $row_pinjam->bunga_pinjaman + $row_pinjam->biaya_adm))); ?></td>
					</tr>
					</table>
				</td>			
			</tr>
	...
	<table width="100%" style="font-size: 17px;">
		<tr>
			...
			<!--<td> Sisa Angsuran : <span id="det_sisa_ags"> <?php echo $row_pinjam->lama_angsuran - $sisa_ags; ?> </span> Minggu </td>-->
			<td> Dibayar : Rp. <span id="det_sudah_bayar"> <?php echo number_format(nsi_round($dibayar)); ?></span> </td>
			<td> Sisa Jasa : Rp. <span id="det_jml_denda"> <?php echo  number_format(nsi_round($sisa_jasa)); ?> </span> </td>
			<td> Sisa Pokok : Rp. <span id="det_jml_denda"> <?php echo  number_format(nsi_round($sisa_pokok)); ?> </span> </td>
...
<thead>
	<tr>
		...
		<th data-options="field:'jml_pokok_angsuran', width:'20', halign:'center', align:'right'"> Pokok Angsuran</th>
		<th data-options="field:'jml_jasa_angsuran', width:'20', halign:'center', align:'center'"> Jasa</th>
	...
	<form id="form" method="post" novalidate>
		<table>
			<tr style="height:35px">
				...
				<td>
					<!--<div class="input-group date dtpicker col-md-5" style="z-index: 9999 !important;">-->
						...
						<input type="text" name="tgl_transaksi" class="dtpicker" id="tgl_transaksi" />
			...
			<tr style="height:30px">
				<td> Angsuran Ke </td>
				<td> : </td>
				<td><input type="text" id="angsuran_keberapa" name="angsuran_keberapa" value=""  /></td>
			</tr>
			<tr style="height:30px">
				<td> Sisa Angsuran </td>
				<td> : </td>
			...
			<tr style="height:30px">
				...
				<td> : </td>
				<td> 
					<input type="text" id="jml_pokok_angsuran" name="jml_pokok_angsuran" value=""  />
				</td>
			</tr>
			...
			<tr style="height:30px">
				...
				<td> : </td>
				<td> 
					<input type="text" id="jml_jasa_angsuran" name="jml_jasa_angsuran" value=""  />
				</td>
			</tr>
			<tr style="height:30px">
				<td> Sisa Tagihan</td>
...
<script type="text/javascript">
	...
	function create() {
		...
		jQuery('#jml_pokok_angsuran').val();
		jQuery('#jml_jasa_angsuran').val();
			...
			success	: function(result){
				...
				if((result.total_tagihan <= 0)) {
					...
					$.messager.show({
						...
						msg: '<div class="text-blue"><i class="fa fa-check"></i>Sudah Lunas</div>',
					});
					...
					$('#ket_lunas').val(result.status_lunas);
				} else {
					$('#angsuran_ke').text(result.ags_ke);
					$('#sisa_ags').text(result.sisa_ags);
					$('#sisa_tagihan').text(result.sisa_tagihan);
		...
		$('#angsuran').keyup(function(){
			...
			$('#angsuran').val(number_format(val_jumlah));
		});
		
		$('#jml_jasa_angsuran').keyup(function(){
			var val_jumlah = $(this).val();
			$('#jml_jasa_angsuran').val(number_format(val_jumlah));
		});
		
		$('#jml_pokok_angsuran').keyup(function(){
			var val_jumlah = $(this).val();
			$('#jml_pokok_angsuran').val(number_format(val_jumlah));
		});
		hitung_denda();
	}
	...
	function save(){
		...
		var berapa_kali_dibayar = $("#berapa_kali_dibayar").val();
			...
			$.ajax({
				...
				success	: function(result){
					...
					if(result.ok) {
						...
						$('#dg').datagrid('reload');
						det_update();
						document.location = "<?php echo base_url("angsuran/index")."/".$master_id; ?>";
					}
				},
		...
		if(row) {
			...
			.done(function(result) {
				if(result.success == '1') {
					...
					$('#sisa_ags').text(result.sisa_ags);
					$('#sisa_tagihan').text(result.sisa_tagihan);					
					$('#jml_jasa_angsuran').text(result.jml_jasa_angsuran);					
					$('#jml_pokok_angsuran').text(result.jml_pokok_angsuran);					
